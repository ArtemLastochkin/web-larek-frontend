...
class ModelCard {
  ...
  getObjCard(id: string) { 
    return this.dataCards.filter((el)=>{return el.id === id})
  }
}

const card = new ModelCard(ObjMain.items);



class CardView {

  constructor(a: ModelCard) {
    const arr: HTMLElement[] = []
    a.dataCards.forEach((e)=>{
      const newElementCard = this.createElementCard(e.category, e.title, e.image, e.price)
      
      arr.push(newElementCard)
  ...
  createElementCard(category: string, title: string, image: string, price: number) {
    const cardCatalogCopy = cloneTemplate(`#card-catalog`)
    cardCatalogCopy.querySelector(`.card__category.card__category_soft`).textContent = `${category}`
    cardCatalogCopy.querySelector(`.card__title`).textContent = `${title}`
    cardCatalogCopy.querySelector(`.card__image`).setAttribute(`src`, `${image}`)
    cardCatalogCopy.querySelector(`.card__price`).textContent = `${price}`
    return cardCatalogCopy
  }
